...
# Copy improved run-vault script with Load Balancer front-end IP address
gsutil cp gs://${vault_bucket_name}/run-vault /opt/vault/bin
chmod 755 /opt/vault/bin/run-vault
chown vault:vault /opt/vault/bin/run-vault
...
/opt/vault/bin/run-vault --gcs-bucket ${vault_bucket_name} --tls-cert-file "$VAULT_TLS_CERT_FILE" --tls-key-file "$VAULT_TLS_KEY_FILE" ${enable_vault_ui} --api-addr "https://${lb_ip}:8200"